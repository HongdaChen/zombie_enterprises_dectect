...
parser.add_argument('--mode', type=str, default='train',
                    help='choose train mode or parameters(only knn and random_forest) mode')
...
if n == 'knn':
    ...
    if momo == 'train':
        ...
        for k in range(1, 14):
            ...
            score = cross_val_score(knn, X, y, cv=6, scoring="accuracy").mean()
...
# ############################################ 逻辑回归###############################################
    ...
    pickle.dump(lr, open("model_logistic.pkl", "wb"))
...
# ############################################# 神经网络 ###########################################
    ...
    pickle.dump(clf, open("model_neural.pkl", "wb"))
...
# ########################################## 随机森林 ##############################################
    ...
    p = Process(base_train_sum, knowledge_train, money_report_train_sum, year_report_train_sum,
                standard=True, features=fe)  # 不进行标准化
    res = p.alpha_process_csv(base_verify1, paient_information_verify1, money_information_verify1, year_report_verify1)
    y = res['flag'].values
    X = res.drop('flag', axis=1)
    train_X, val_X, train_y, val_y = train_test_split(X, y, test_size=0.3, random_state=0)
